...
def get_args(parser):
    ...
    parser.add_argument("--loaddir",      type=str,
                        default="output/YOUR_PRETRAIN_CHECKPOINT")
    ...
    parser.add_argument("--init_model",        type=str,
                        default="bert-base-scratch")
    parser.add_argument("--dropout",           type=float, default=0.1)
    parser.add_argument("--num_image_embeds",  type=int,   default=64)
    parser.add_argument("--img_embed_pool_type", type=str, default="max")
    # img_hidden_sz matches BioMedCLIP output after projection = 768
...
def get_criterion(args, device):
    if args.task_type == "multilabel":
        if args.weight_classes:
            ...
            weights = (torch.FloatTensor(freqs) /
    ...
    for name, p in model.named_parameters():
        ...
        if 'img_encoder' in name:
            img_params.append(p)
                 ...
                 ['txt_embeddings', 'encoder.layer', 'pooler']):
...
def model_eval(data, model, args, criterion, device, store_preds=False):
    ...
    with torch.no_grad():
        ...
        for batch in tqdm(data, desc="Evaluating"):
            loss, out, tgt = model_forward(model, args, criterion,
                                           batch, device)
            losses.append(loss.item())
...
def model_forward(model, args, criterion, batch, device):
    txt, segment, mask, img, tgt = batch
    print(f"txt shape: {txt.shape}, img shape: {img.shape}")
    txt, img   = txt.to(device), img.to(device)
    mask       = mask.to(device)
...
def train(args):
    ...
    # Load pretrained MedViLL weights (partial — image + bert side)
    ...
    for epoch in range(args.max_epochs):
        ...
        # Unfreeze encoder after 3 warmup epochs
        if epoch == 3:
        ...
        for step, batch in enumerate(tqdm(train_loader,
            ...
            loss, _, _ = model_forward(model, args, criterion,
                                       batch, device)
            if args.gradient_accumulation_steps > 1:
                loss = loss / args.gradient_accumulation_steps
        ...
        # Evaluate
        metrics, classACC, tgts, preds = model_eval(
            val_loader, model, args, criterion, device)

        logger.info(f"EP{epoch} Train Loss: {np.mean(train_losses):.4f}")
        log_metrics("Val", metrics, args, logger)
        ...
        if is_improvement:
            ...
            print(f"[SAVED] Best model EP{epoch} "
        ...
        # Save per-epoch CSV
        save_path = os.path.join(args.savedir, f"{args.save_name}.csv")
        with open(save_path, 'w', newline='') as f:
            wr  = csv.writer(f)
            keys = list(classACC.keys())
            vals = list(classACC.values())
            wr.writerow(['micro_auc', 'macro_auc',
                         'micro_f1',  'macro_f1'] + keys)
            wr.writerow([
                metrics.get("micro_roc_auc", 0),
                metrics.get("macro_roc_auc", 0),
